...
const DoorSeam: React.FC<{ side: 'left' | 'right' }> = ({ side }) => (
  <div style={{
    ...
    <div style={{
      ...
      background: 'linear-gradient(to bottom, transparent 0%, rgba(233,187,210,0.08) 4%, rgba(233,187,210,0.9) 32%, rgba(255,235,245,1) 50%, rgba(233,187,210,0.9) 68%, rgba(233,187,210,0.08) 96%, transparent 100%)',
      boxShadow: `0 0 10px 4px rgba(233,187,210,0.9), 0 0 30px 12px rgba(233,187,210,0.7), 0 0 70px 28px rgba(233,187,210,0.5), 0 0 120px 45px rgba(233,187,210,0.25)`,
    ...
    {_SEAM_PX.map((p, i) => {
      ...
      return (
        <div key={i} style={{
          ...
          boxShadow: `0 0 ${p.glow}px ${Math.round(p.glow / 2)}px rgba(233,187,210,0.45)`,
          animation: `sp-fly ${p.dur} ease-out ${p.delay} infinite`,
          willChange: 'transform, opacity',
          transform: 'translate(-50%, -50%)',
          ['--sx' as any]: '0px', ['--sy' as any]: '0px',
          ['--ex' as any]: `${finalEx}px`, ['--ey' as any]: `${p.ey}px`,
        }} />
      );
    })}
  </div>
...
const CastleOverlayText: React.FC<{
  ...
  const nameTop = isStaticPreview ? '38%' : (editMode ? '18%' : '50%');
  ...
  const inviteTopPos = isStaticPreview ? '78%' : (editMode ? '52%' : '50%');
  const inviteTransform = isStaticPreview ? 'translateY(-50%) scale(0.95)' : (editMode ? 'translateY(-50%) scale(1)' : 'translateY(-50%) scale(0.88)');
  const inviteOpacity = isStaticPreview ? 1 : (editMode ? 1 : 0);
  return (
  <div ref={overlayRef} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 15, pointerEvents: editMode ? 'auto' : 'none' }}>
    ...
    <div style={{ position: 'absolute', top: '15%', left: 0, right: 0, textAlign: 'center', zIndex: 1 }}>
      {editMode ? (
        <InlineEdit
          ...
          editMode
          value={welcomeText}
          onChange={v => onWelcomeChange?.(v)}
          textKey="intro:welcome"
          textLabel="Intro Welcome"
          style={{ fontFamily: 'Cinzel, serif', fontSize: '1rem', fontWeight: 700, letterSpacing: '0.5em', textTransform: 'uppercase', color: tc.gold, textShadow: `0 0 14px ${tc.gold}aa` }}
        />
      ) : (
    ...
    <div style={{ position: 'absolute', top: nameTop, left: 0, right: 0, transform: nameTransform, textAlign: 'center', zIndex: 1, padding: '0 28px', opacity: nameOpacity }}>
      <div ref={nameRef} style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center', width: '100%' }}>
        <InlineEdit
          tag="h2"
          editMode={!!editMode}
          value={childName}
          onChange={v => onChildNameChange?.(v)}
          textKey="intro:name"
          textLabel="Intro Name"
          style={{ fontFamily: INTO_TEXT, fontSize: '5.2rem', lineHeight: 1.15, color: tc.pinkL, textShadow: `0 2px 24px rgba(0,0,0,0.85), 0 0 40px ${tc.gold}55`, margin: '2px auto 0', maxWidth: '100%', whiteSpace: 'normal', overflowWrap: 'anywhere', wordBreak: 'break-word', textWrap: 'balance' }}
        />
        <InlineEdit
          tag="p"
          editMode={!!editMode}
          ...
          style={{ fontFamily: 'Cinzel, serif', fontSize: '1rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: tc.pinkL, textShadow: '0 2px 10px rgba(0,0,0,0.75)', marginTop: 2 }}
        />
      </div>
    </div>
...
const CastleIntro: React.FC<{
  ...
  themeColors?: { pinkDark: string; pinkL: string; pinkXL: string; gold: string };
        ...
        previewMode = 'doors', childName = '', subtitle = 'in my castle', welcomeText = 'WELCOME',
        inviteTop, inviteMiddle, inviteBottom, inviteTag, dateStr,
        onChildNameChange, onSubtitleChange, onWelcomeChange,
        onInviteTopChange, onInviteMiddleChange, onInviteBottomChange, onInviteTagChange,
    ...
    return (
      <div style={{ position: 'relative', height: 800, borderRadius: 12, marginBottom: 32, overflow: 'hidden' }}>
        ...
        <CastleOverlayText
          ...
          themeColors={themeColors} previewMode={previewMode} />
      </div>
    );
  }
...
const CastleMagicTemplate: React.FC<InvitationTemplateProps & {
  ...
  // ── Apply color theme ───────────────────────────────────────────────────────
  const activeColorTheme = (pr as any).colorTheme || globalConfig.colorTheme || 'default';
  const theme = getCastleTheme(activeColorTheme);
