...
const Canvas: React.FC<CanvasProps> = memo(({ 
  ...
  selectedId, setSelectedId, placementMode, onPlace, onDelete, onSeatClick, onMoveGuest, onUpdateCapacity, lang
}) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [isPanning, setIsPanning] = useState(false);
  const [dropTargetId, setDropTargetId] = useState<string | null>(null);
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 }); // Track mouse for ghost element
  ...
  // --- Auto-Center on Mount (Fixed Size World) ---
  useEffect(() => {
    if (containerRef.current) {
        ...
        setConfig(prev => ({
            ...
            panX: initialPanX,
            panY: initialPanY,
            scale: rect.width < 768 ? 0.5 : 1 // Pe mobil pornim cu zoom out (0.5), pe desktop normal (1)
        }));
    }
  ...
  // --- TOUCH EVENTS (PANNING) ---
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    if (e.touches.length === 1) {
       // Allow panning if touching the background
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    if (isPanning && e.touches.length === 1 && touchRef.current) {
        // e.preventDefault() is implicitly handled by style touch-action: none
  ...
  return (
    <div 
        ...
        style={{ touchAction: 'none' }} // Prevents browser zoom/scroll gestures
    ...
    >
      ...
      <div style={{ transform: `translate3d(${config.panX}px, ${config.panY}px, 0) scale(${config.scale})`, width: config.width, height: config.height, transformOrigin: '0 0' }} className="relative pointer-events-none">
        ...
        {/* FIXED PAPER BACKGROUND LAYER - Grid Pattern Added */}
        <div 
        ...
        {sortedElements.map(el => {
          ...
          return (
            <CanvasItem 
              ...
              onSeatClick={onSeatClick} onDragOver={id => setDropTargetId(id)} onDragLeave={() => setDropTargetId(null)} onDrop={(toId, data) => { setDropTargetId(null); const { fromElId, fromIdx } = JSON.parse(data); onMoveGuest(fromElId, fromIdx, toId); }} onUpdateCapacity={onUpdateCapacity} 
            />
          );
        })}

        {/* GHOST ELEMENT FOR PLACEMENT */}
        {placementMode && (
          <CanvasItem
            el={{
