...
const CanvasItem = memo(({ 
  ...
  onSelect, onDelete, onRotateStart, onResizeStart, onSeatClick, onDragOver, onDragLeave, onDrop, onUpdateCapacity
}: CanvasItemProps) => {
    ...
    <>
      <div
        ...
        onMouseDown={e => {
          ...
          if (target.closest('.seat-handle') || target.closest('.rotate-handle') || target.closest('.delete-handle') || target.closest('.resize-handle') || target.closest('.control-bar')) return;
          e.stopPropagation();
      ...
      >
        ...
        {hasSeats && el.guests?.map((guest: Guest | null, idx: number) => {
          ...
          // Color coding for kids or special diets
          ...
          return (
            <div 
              ...
              onClick={e => { if (!isGhost) { e.stopPropagation(); onSeatClick?.(el.id, idx); } }} 
              onMouseEnter={(e) => {
                  if (guest && !isGhost) {
      ...
      {/* PORTAL FOR TOOLTIPS - RENDERS OUTSIDE OF TRANSFORMED PARENT */}
